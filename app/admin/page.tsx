...
import {
  ...
  SkipForward, Loader2, UserMinus, RotateCcw, Plus, Play, ShieldCheck, Clock, X
} from 'lucide-react';
...
export default function AdminPage() {
  ...
  console.log('[Admin] Render State:', { mounted, socketConnected: !!socket, isAuthenticated });

  if (!mounted) return (
    <div className="min-h-screen bg-[#0f1115] flex items-center justify-center">
      <Loader2 className="w-8 h-8 animate-spin text-orange-500 opacity-20" />
    </div>
  );

  if (!socket) return (
    <div className="min-h-screen bg-[#0f1115] flex items-center justify-center text-white/20">
      ...
      >
          ...
          <div className="bg-black aspect-video rounded-[1.5rem] lg:rounded-[2.5rem] overflow-hidden border border-white/10 relative shadow-2xl shadow-orange-500/5">
            {currentVideo?.videoId ? (
              <Suspense fallback={<PlayerFallback />}>
                <ReactPlayer
                  src={`https://www.youtube.com/watch?v=${currentVideo.videoId}`} // dont touch this line!!!
                  ref={(player: any) => {
        ...
        <aside className={`${mobileActiveTab === 'queue' ? 'block' : 'hidden lg:block'} space-y-6 lg:mt-0 lg:sticky lg:top-24 lg:h-[calc(100vh-120px)] lg:flex lg:flex-col`}>
          ...
          <div className="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar">
            <DndContext id="admin-dnd-context"
              sensors={sensors}
                ...
                >
                  ...
                  <div className="relative group">
                    ...
                    <input
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-[1.5rem] py-4 lg:py-5 pl-14 pr-12 outline-none focus:border-orange-500/50 focus:ring-4 focus:ring-orange-500/10 transition-all text-base lg:text-xl font-medium"
                      value={searchQuery}
                    ...
                    {searchQuery && (
                      <button
                        ...
                        className="absolute right-14 top-1/2 -translate-y-1/2 p-2 hover:bg-white/10 rounded-xl transition-all text-white/20 hover:text-white"
                        title="Clear Search"
                      >
                        <X className="w-5 h-5" />
                      </button>
                  ...
                  <div className="bg-white/[0.02] border border-white/10 rounded-[1.5rem] overflow-hidden">
                    <div className="grid sm:grid-cols-2 gap-3 max-h-[50vh] lg:max-h-[60vh] overflow-y-auto custom-scrollbar p-3">
                      <AnimatePresence mode="popLayout">
                        {searchResults.map((video) => (
                          <motion.div
                            ...
                            layout
                            className="flex items-center gap-4 p-3 bg-white/[0.03] border border-white/5 rounded-2xl group hover:border-orange-500/30 hover:bg-white/[0.05] transition-all"
                          >
                            <div className="relative shrink-0">
                              <img src={video.thumbnail} alt="" className="w-24 lg:w-36 h-14 lg:h-20 object-cover rounded-xl shadow-lg" />
                              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl flex items-center justify-center">
                                <Music2 className="w-5 h-5 text-white" />
                              </div>
                            </div>
                            <div className="flex-1 min-w-0">
                              <h4 className="font-bold line-clamp-2 text-xs lg:text-base group-hover:text-orange-500 transition-colors italic mb-2">{video.title}</h4>
                              <div className="flex gap-2">
                                <button
                                  onClick={() => handlePlayNow(video)}
                                  className="flex-[2] py-2 bg-white text-black rounded-lg text-[9px] font-black uppercase tracking-widest hover:scale-[1.02] active:scale-[0.98] transition-all shadow-lg"
                                >
                                  Play
                                </button>
                                <button
                                  onClick={() => handleAdminAdd(video)}
                                  className="flex-[2] py-2 bg-orange-500 text-black rounded-lg text-[9px] font-black uppercase tracking-widest hover:scale-[1.02] active:scale-[0.98] transition-all shadow-lg shadow-orange-500/20"
                                >
                                  Add
                                </button>
                                <button
                                  onClick={() => handleBanVideo(video.videoId, video.title)}
                                  className="flex-1 py-2 bg-white/5 hover:bg-orange-500/10 text-white/20 hover:text-orange-500 border border-white/10 rounded-lg transition-all flex items-center justify-center"
                                  title="Ban Video"
                                >
          ...
          {confirmDialog.isOpen && (
            <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
              ...
              <motion.div
                ...
                animate={{ opacity: 1, scale: 1, y: 0 }}
                exit={{ opacity: 0, scale: 0.95, y: 20 }}
                className="relative w-full max-sm bg-[#151619] border border-white/10 rounded-[2rem] p-6 lg:p-8 space-y-6 shadow-2xl overflow-hidden"
              >
                <div className="space-y-2">
